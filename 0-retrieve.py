...
out_path_accidents = os.path.join(data, 'accidents.csv')
...
out_path_segments = os.path.join(data, 'segments.zip')
    ...
    for year in range(2012, 2019+1):
        for month in range(1, 12+1):
            weather_df.append({
                ...
                'year': year,
                'month': month,
                'out_path': os.path.join(data, 'weather_'+str(station)+'_'+str(year)+'_'+str(month)+'.csv'),
...
out_path_weather_df = os.path.join(data, 'weather.csv')
if os.path.isfile(out_path_weather_df) == False:
    ...
    logging.info(f'Finished downloading {out_path_weather_df}')
else:
    logging.info(f'Already downloaded {out_path_weather_df}')
